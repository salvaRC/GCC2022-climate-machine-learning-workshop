...
from src.datamodules.oisstv2_datamodule import OISSTv2DataModule
...
def get_datamodule(config: DictConfig) -> OISSTv2DataModule:
    ...
    .. code-block:: python
        ...
        cfg = get_config_from_hydra_compose_overrides(overrides=['datamodule=oisstv2', 'datamodule.batch_size=32'])
...
def get_model_and_data(config: DictConfig) -> (BaseModel, OISSTv2DataModule):
    ...
    Returns:
        (BaseModel, OISSTv2DataModule): A tuple of (model, datamodule), that you can directly use to train with pytorch-lightning
    ...
    .. code-block:: python
        ...
        cfg = get_config_from_hydra_compose_overrides(overrides=['datamodule=oisstv2', 'model=mlp'])
